...
public class IdariMemurVerilenDersEkleSil extends JPanel implements Etkinlestirilebilir{
	...
	IdariMemurVerilenDersEkleSil(IdariMemur im,IdariMemurForm form){
		...
		ogretimuyesi= new JButton("Dersi Verecek Öðretim Üyesini Seçmek Ýçin Týklayýnýz.");
		...
		sil.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int option=JOptionPane.showConfirmDialog(ths, "Bu Öðretim Üyesinin Bu Dersi Verme Kaydýný Silmek Ýstediðinize Emin Misiniz ?","Silme Ýþlemi",JOptionPane.YES_NO_OPTION);
				if(option==JOptionPane.YES_OPTION) {
					...
					JOptionPane.showMessageDialog(ths, "Verilen Ders Kaydý Baþarýyla Silindi.","Silindi!",JOptionPane.INFORMATION_MESSAGE);
		...
		ekle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(ths, "Öðretim Üyesinin Dersi Verme Kaydý Yapýldý.","Kaydedildi!",JOptionPane.INFORMATION_MESSAGE);
			...
			list.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					if(e.getClickCount()==2) {
						...
						pnl.ogretimuyesi.setText("Seçilen Öðretim Üyesinin Tc Kimlik Numarasý: "+ou.tcKimlik+" | Baþka Bir Öðretim Üyesi Seçmek Ýçin Týklayýnýz.");
						setAllEnabled(true);
					}
				}
			});
