...
public class OgrenciDersProgrami extends JPanel{
	...
	Ogrenci ogr;
	public OgrenciDersProgrami(Ogrenci ogr) {
		super();
		...
		for(int i=0;i<vakitList.size();i++) {
			...
			if(gun.equals("PAZARTESÝ"))
				...
				sutun=7;
			program.setValueAt(dersList.get(vakitList.get(i).index).kod+"-"+dersList.get(vakitList.get(i).index).ad, satir, sutun);
		}
